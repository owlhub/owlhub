...
export default function NewIntegrationPage() {
  ...
  const fetchAppDetails = async (appId: string) => {
    ...
    try {
      ...
      const billing = allFindings.filter((finding: AppFinding) => (finding.type === 'billing' || finding.type === 'cost'));
      const other = allFindings.filter((finding: AppFinding) => finding.type !== 'posture' && finding.type !== 'billing' && finding.type !== 'cost');
    ...
    if (selectedApp) {
      selectedApp.configFields.forEach(field => {
        if (field.type === 'boolean') {
          // Set default value for boolean fields (default to false if not specified)
          newConfigValues[field.name] = field.default === true ? 'true' : 'false';
        }
      });
    }
                ...
                {field.type === 'boolean' ? (
                  <div className="flex items-center">
                    <button
                      ...
                      id={field.name}
                      disabled={submitting || field.editable === false}
                    >
                      <span
